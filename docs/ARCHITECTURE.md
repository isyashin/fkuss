...
- **Печатные материалы**: данные ресторана + Settings(printMaterials) →
  ...
  SVG, PNG 300 dpi или PDF. QR URL принудительно берётся из canonical тенанта;
  export ограничивает размер/rate/concurrency, а логотип читается только из
  реального файла внутри content/. Макеты содержат вылеты 3 мм; настройки
  хранятся отдельно для визитки и магнита без новой таблицы БД.
