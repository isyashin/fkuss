...
Админка каждого ресторана использует свои таблицы `AdminUser` и
`AdminSession` в изолированной БД сайта. `owner` управляет всеми разделами и
сотрудниками, `staff` — заказами и бронями. Пароли хранятся как scrypt-хеши с
индивидуальной солью; cookie содержит случайный токен, а БД — только его хеш.
Первого владельца создаёт одноразовая bootstrap-команда после миграции БД.
Новые заказы с оплатой при получении, брони и впервые оплаченные онлайн-заказы
создают `AdminEvent` в той же транзакции. Открытая админка опрашивает БД раз в
10 секунд; `AdminEventReceipt` отмечает выдачу события каждому сотруднику,
чтобы повторный опрос и перезагрузка не включали звук снова. Выбор сигнала
лежит в `Settings` текущего сайта, свой аудиофайл — в его `CONTENT_DIR/audio/`.
Системных уведомлений после закрытия вкладки нет.
Новый заказ сохраняет выбранный гостем способ связи в `Order.preferredChannel`.
Админка строит ручные ссылки из проверенного международного номера; WhatsApp
открывается через `wa.me` без текста сообщения. Для старых заказов без выбора
и для броней доступны оба разрешённых в настройках канала. Если выбранный
канал скрыт, он не заменяется другим мессенджером.
...
витрина/админка. Админка правит БД напрямую; `content/` остаётся смонтированным
для медиафайлов. Для импортов и ручного наполнения provenance и нерешённые
...
4. Если выбрана онлайн-оплата — PaymentProvider.createPayment, редирект
   на оплату; подтверждённый webhook сразу переводит новый заказ в «Принят».
5. «Оплата при получении» — заказ сразу уходит в каналы.

## Поток бонусов

- `BonusTransaction`: type = accrual | spend | reversal, orderId, amount.
- Начисление — при финале «Доставлен» для доставки или «Выдан» для
  самовывоза (X% от суммы, не включая оплаченную бонусами часть).
