...
test("три соуса: сумма = блюдо + все три соуса, снимок содержит все варианты", async ({ request }, testInfo) => {
  ...
  const headers = { "x-forwarded-for": `10.99.4.${testInfo.project.name === "webkit-mobile" ? 13 : testInfo.project.name.includes("mobile") ? 11 : 12}` };
...
test("четвёртый соус сверх max=3 отклоняется сервером", async ({ request }, testInfo) => {
  const headers = { "x-forwarded-for": `10.99.5.${testInfo.project.name === "webkit-mobile" ? 13 : testInfo.project.name.includes("mobile") ? 11 : 12}` };
...
test("чужой modifierId отклоняется сервером", async ({ request }, testInfo) => {
  const headers = { "x-forwarded-for": `10.99.6.${testInfo.project.name === "webkit-mobile" ? 13 : testInfo.project.name.includes("mobile") ? 11 : 12}` };
